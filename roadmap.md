...
- [x] **تصفير تسريب الإجابات بين الأسئلة**: من 111 زوجاً إلى صفر. أُعيدت كتابة 31 عبارة من سباق السرعة (بتوازن 25/25 محفوظاً)، وأكثر من ستين معلومة وسؤالاً وتلميحاً. وما بقي بعد ذلك كان ازدواجاً حقيقياً — سؤالان يختبران الحقيقة نفسها بصيغتين — فغُيّر أحد الطرفين: لغز غريان صار عن الجرة لا الفخار، ولغز مصراتة عن السجاد لا النادي، ولغز سرت عن الهلال النفطي لا الخليج، وأدلة المتقاطعة لم تعد تعيد طرح أسئلة البنك. **القائمة فارغة الآن، فأي تسريب جديد يُفشل البناء.**
...
**أُغلق البندان:** لم يبقَ سؤال صعب بلا مصدر إلا واحداً، وكل مدينة صار لها ثلاثة أو أربعة أسئلة خاصة بها بعد أن كانت بني وليد بسؤال واحد.

**كيف:** بتفويض البحث إلى Antigravity CLI في جلستين متوازيتين، بعقد أدلة صارم: كل نتيجة تحمل **دعوى مكتوبة** و**اقتباساً حرفياً** من صفحة فُتحت فعلاً، و**إقراراً** بأن الاقتباس يذكر الدعوى. ومُنع الوكيل من الكتابة في `src/` إطلاقاً — يكتب ملف نتائج، والمراجعة والتطبيق من المنسّق. وقد استحق هذا الحرص: ثبت اختلاق مصدر كامل لمثل شعبي، وثبت أن «معلومة» عن نادي الزاوية تنسب إليه اسماً هو أحد الخيارات الخاطئة في السؤال نفسه، وأُسقطت خمسة أسئلة لأنها تكرار لأسئلة قائمة.

---

## ♿ المرحلة 11 — إتاحة وخصوصية وسنة كاملة من الألغاز (2026-08-15)

### 🔒 إخراج رقم التواصل من الشفرة

- [x] قناة الإبلاغ صارت **من بيئة البناء** لا مكتوبة في الملفات. الرقم المدمج في الحزمة يقرؤه كل من يفتح الموقع، وسرية المستودع لا تغيّر ذلك بعد النشر.
- [x] **يفشل مغلقاً**: إن لم تُضبط قناة، لا يظهر رابط إبلاغ أصلاً بدل الرجوع إلى رقم شخصي. وتُحُقّق من ذلك ببناء الحالتين: مع تعطيل المتغيّر لا يبقى في `dist` لا الرقم ولا اسم المضيف.

### ⌨️ إتاحة فعلية لا معلنة

- [x] **النوافذ المنبثقة تحتجز التركيز فعلاً**: أربع نوافذ كانت تعلن `aria-modal` بينما يخرج Tab إلى الخريطة خلفها. [useModalA11y](file:///c:/Users/masal/Documents/opencode/trivia-game/src/hooks/useModalA11y.ts) يدخل التركيز ويدوّره ويغلق بـ Escape ويعيده لمن فتح.
- [x] **الخيارات تُقرأ صحيحة**: كان حرف الترتيب يلتصق بالإجابة فيُنطق «أحرب القرم» كلمة واحدة.
- [x] **تكبير الصفحة أُعيد**: قفل التكبير على شاشة أكثرها نص عربي يضر من يحتاج التكبير، وتكبير الخريطة بإصبعين لم يكن يعتمد عليه — تُحُقّق منه بعد التغيير.
- [x] **تفضيل تقليل الحركة صار محترماً حقاً**: قاعدة CSS وحدها لا تمس Framer Motion لأنه يكتب أنماطاً مضمّنة إطاراً بإطار، وأربعة عشر مكوّناً تتحرك به. قيست الحركة قبل وبعد: من أربعة إطارات وسيطة إلى صفر.

### 📅 سنة كاملة قبل أن يعود شيء

- [x] **الألغاز من 30 إلى 52، والشبكات من 18 إلى 52** — واحد لكل أسبوع من السنة. أقصر فاصل قبل تكرار أي لغز صار **364 يوماً** بعد أن كان 126، مقيساً بمشي المولّد 900 يوم.
- [x] **عتبة الفحص من 60 إلى 300 يوم** حتى لا تتآكل السنة بصمت إن حُذف لغز.
- [x] **هندسة الشبكات تُحلّ بالبحث** في [crossword-build.mjs](file:///c:/Users/masal/Documents/opencode/trivia-game/scripts/crossword-build.mjs)، والعمل البشري يبقى على الكلمات والأدلة.
- [x] **ثلاثة عشر لغزاً مسوّداً أُسقطت لا أُعيدت صياغتها**: الفحص أبلغ عنها تسريباً، وقراءتها تقول إنها تكرار — البنك يسأل أصلاً عن واحات الجفرة وعن القديد وعن الزنقة وعن زاوية الجغبوب.
- [x] **لوحة المفاتيح قورنت بالفعلية**: قائمة الفاحص نسخة من قائمة المكوّن، فشبكة تحتاج حرفاً لا يُكتب كانت ستمر. القائمتان متطابقتان، وكل حرف في الـ52 شبكة يُكتب.
- [x] **مشاركة الألغاز المحلولة**: كانت شاشتا اللغز والشبكة الوحيدتين بلا بطاقة مشاركة، والبطاقة تحمل النتيجة لا الإجابة.

---

## المرحلة 10: معايرة الرتب، الصعوبة المرئية، وتراث «معلومة ع الماشي» (15 أغسطس 2026)

### 👑 معايرة الرتب وحارس النجوم (ق-1 / ح-2)
- [x] **معايرة عتبات الرتب على 285 نجمة** (95 مرحلة × 3 نجوم = 9,975 نقطة من الخريطة وحدها):
  - `novice` (مستكشف مبتدئ): 0 نقطة
  - `guide` (دليل الدروب): 500 نقطة
  - `traveller` (رحّالة المدن): 1,500 نقطة
  - `keeper` (حافظ التراث): 3,200 نقطة
  - `sage` (جهبذ الأقاليم): 5,800 نقطة
  - `master` (شيخ الدبارة): 9,000 نقطة (~87-90% من إجمالي الخريطة والأداء)
- [x] **حارس آلي لانحراف النجوم**: فحص تلقائي في `scripts/questions-check.mjs` يقارن نجوم مراحل `src/data/cities.ts` مع `TOTAL_MAP_STARS` المستورد من `src/data/ranks.ts` ويمنع أي انحراف مستقبلي.

### ⏱️ مستويات الصعوبة والمؤقت الديناميكي (ب-1)
- [x] **شارة الصعوبة والمكافأة**: إظهار مستوى السؤال (سهل 🟢، متوسط 🟡، صعب 🟠، خبير 🟣) وقيمة الجائزة المالية على بطاقة السؤال في `QuizScreen`.
- [x] **مؤقت متدرج وعتبات نجوم تناسبية**:
  - سهل: 20 ثانية (3 نجوم: >=15ث، نجمتان: >=9ث)
  - متوسط: 25 ثانية (3 نجوم: >=19ث، نجمتان: >=12ث)
  - صعب: 30 ثانية (3 نجوم: >=22ث، نجمتان: >=14ث)
  - خبير: 35 ثانية (3 نجوم: >=25ث، نجمتان: >=16ث)

### ⌨️ توحيد لوحة مفاتيح الكلمات المتقاطعة (ح-3)
- [x] **مصدر واحد موحد**: إنشاء `src/data/puzzles/crosswordKeyboard.ts` يحوي مصفوفة الـ 33 حرفاً المتناسقة، واستيراده المشترك بين `MiniCrossword.tsx` وفاحص النزاهة `questions-check.mjs`.

### 📖 دفتر المعارف «معلومة ع الماشي» (ب-4)
- [x] **سجل دائم للمعارف التراثية**: إضافة تبويب «دفتر المعارف» داخل شاشة الأوسمة والرتب (`BadgesScreen`).
- [x] **بحث وتصفية سريعة**: إمكانية البحث الفوري في نصوص المعلومات والأسئلة والمصادر، والتصفية حسب التصنيفات التراثية (تاريخ، لهجات، مأكولات، جغرافيا، إسلاميات...).
- [x] **مشاركة المعلومات**: إمكانية نسخ ومشاركة أي معلومة تراثية مصممة مع أصدقاء اللاعب عبر واجهات المشاركة.

### ⚡ تجزئة الحزم وتخفيف حزمة الإقلاع (ب-2)
- [x] **تحميل كسول لكافة الشاشات غير الأساسية**: استخدام `React.lazy` و `Suspense` مع مؤشر انتظار تراثي `HeritageLoadingFallback`.
- [x] **فصل بنوك الأسئلة والألغاز**: تكوين `manualChunks` لعزل `questions-bank` (~241 ك.ب) و `puzzles-bank` (~70 ك.ب).
- [x] **تقليص حزمة الإقلاع بنسبة 78.6%**: انكماش `dist/assets/index.js` من **526.3 ك.ب** إلى **112.8 ك.ب** (من 132.7 ك.ب إلى 31.9 ك.ب مضغوطة).
- [x] **استمرارية التخزين المؤقت الكامل في Service Worker**: كشف وتخزين كافة الـ 18 أصل مبني في `sw.js` واجتياز اختبارات الأوفلاين 100%.

### 🔍 تدقيق المصادر الحية والتوسع المتوازن (ح-4 & ب-3)
- [x] **فحص 100% لمقالات ويكيبيديا عبر API**: فحص وتأكيد كافة المقالات الـ 113 عبر `npm run sources:audit`.
- [x] **موازنة تصنيف المطبخ والعادات**: إضافة 16 سؤالاً موثقاً ليصل تصنيف `food_traditions` إلى 40 سؤالاً متكاملاً وارتفاع إجمالي الأسئلة إلى 409 أسئلة بصفر تسريب.

### 🌟 خط أساس Lighthouse وتدقيق قارئ الشاشة (ح-1 & ح-5)
- [x] **درجات Lighthouse القياسية (Mobile Emulation)**:
  - ♿ إمكانية الوصول (Accessibility): **100 / 100**
  - 🛡️ أفضل الممارسات (Best Practices): **100 / 100**
  - 🔍 تحسين محركات البحث (SEO): **100 / 100**
  - ⚡ الأداء (Performance): **75 / 100** (مع 40ms TBT و 0 CLS)
- [x] **جناح اختبارات الإتاحة وقارئ الشاشة**: إنشاء `tests/a11y.spec.ts` واجتياز كافة الـ 30 اختباراً آلياً بنسبة 100%.

### ✨ حزمة المؤثرات التفاعلية والجماليات البصرية (Micro-delight)
- [x] **انفجار العملات والنجوم الذهبية**: مكوّن `RewardCelebration.tsx` يطلق 24 جسيماً ذهبياً وتفاعلياً عند الفوز بـ 3 نجوم في المسابقة أو حل الألغاز، مع نبضات لمسية (Haptic Feedback) على الهواتف.
- [x] **قوافل التجارة المتحركة على الخريطة**: تأثير مسارات متدفقة `animate-caravan-flow` تعبر بين المدن والواحات المفتوحة في `LibyaVectorMap.tsx`.
- [x] **هالة توهج ونبض للمدن المكتملة**: تأثير `animate-beacon-glow` يبرز المدن المنجزة بنجومها الثلاث.
- [x] **أزرار ثلاثية الأبعاد بلمسة فيزيائية**: فئة `tactile-button` للاستجابة اللمسية الفورية عند النقر على الخيارات ولوحة المفاتيح.

### 🚩 نافذة ونموذج التبليغ الذكي المدمج (ق-2)
- [x] **نافذة منبثقة مدمجة**: إنشاء `ReportModal.tsx` بنمط Neo-Heritage لعرض استمارة التبليغ دون مغادرة اللعبة.
- [x] **تكامل Tally واستبدال المعاملات تلقائياً**: استبدال `{subject}` و `{text}` آلياً لتضمين معرّف السؤال `#id`.
- [x] **دعم الأوفلاين 100%**: إمكانية نسخ تفاصيل السؤال بنقرة واحدة عند غياب الإنترنت.

---

### 🚀 حالة المشروع (Project Status: Production Ready)

كافة معالم المراحل مكتملة، وتم التحقق من كل أنماط اللعب فعلياً في المتصفح واختبارات E2E الآلية بنجاح 100%.

**أرقام المحتوى الحالية:**

| المحتوى | العدد |
| --- | --- |
| أسئلة الاختيار من متعدد | 409 أسئلة في 9 تصنيفات (40+ لكل تصنيف) |
...
| ألغاز ترتيب الحروف | 52 لغزاً |
| شبكات الكلمات المتقاطعة | 52 شبكة |
| أسئلة موثّقة بمصدر | 173 سؤالاً (100% من الصعب والخبير موثق) |
| أسئلة بتلميح | 142 من 143 صعباً وخبيراً |
| أزواج تسريب مؤجلة | صفر (اجتياز فاحص النزاهة 563 سجلاً) |
...
**الخطوات التالية:** مرتّبة ومعلّلة في [next-steps.md](./next-steps.md) — وعلى
رأسها قراران لا يصنعهما غيرك: معايرة الرتب على 285 نجمة بدل 231، ونموذج تبليغ
يغني عن نشر رقم شخصي.
