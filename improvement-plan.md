...
> وُضعت هذه الخطة بعد مراجعة كاملة لبنوك الأسئلة (265 سجلاً) وقراءة مسارات
> اللعب في الشيفرة. كل بند فيها مبني على شاهد من الملفات، لا على تخمين؛
> والشاهد مذكور مع كل بند حتى يمكن مراجعته أو نقضه.
...
| --- | --- | --- |
| اختيار من متعدد | 219 | 9 تصنيفات — كانت 195 |
...
**ما أُنجز في جولة المراجعة الأخيرة:** تصحيح أخطاء واقعية، وإزالة بديل ملتبس،
وإعادة معايرة الصعوبة على كل البنك (67 سهل / 74 متوسط / 40 صعب / 14 خبير)،
وتوزيع موقع الإجابة (50/50/48/47 بعد أن كان 50% منها في الموضع الثاني)،
وبناء فحص محتوى آلي `npm run questions:check` وصفحة مراجعة تفاعلية.
...
**الشاهد:** جولة اللعب السريع تسحب 5 أسئلة عشوائية من تصنيف واحد
([CategoryHub.tsx:145](src/features/quickplay/CategoryHub.tsx#L145))، وأصغر
التصنيفات: أدب 16، ثقافة عامة 16، إسلامية 16، علوم 18، رياضة 20. أي أن جولتين
أو ثلاثاً تستهلكان التصنيف كله، ثم يبدأ التكرار المحسوس.
...
- **لا تكرار قبل استنفاد المجموعة**: كل نوع يمشي في مجموعته بخطوة أولية
  ...
  800 يوم: أول تكرار سؤال بعد **341 يوماً** (بعد استعمال الـ195 كلها)، وأول
...
عشر في **م-3**. النمط الأسبوعي الآن: 3 معلومة / 2 سرعة / 1 حروف / 1 متقاطعة.
...
### ز-1 — التدرّب على الأخطاء ⬤ أعلى قيمة تعليمية
...
**المقترح:** `missedIds` و`seenIds` في المخزن، ثم:

- نمط **«تدرّب على أخطائك»** يعيد ما أُخطئ فيه بفاصل زمني متباعد.
- **منع التكرار**: الجولة تسحب من غير المرئي أولاً، فلا يتكرر سؤال قبل استنفاد
  التصنيف (يعالج نصف مشكلة **م-1** فوراً وبتكلفة أقل).

كلا الحقلين مصمم ليُزامن مع الخادم لاحقاً دون إعادة بناء.
...
- [ ] **ربط الفحص بالبناء** (`build: questions:check && tsc -b && vite build`)
      حتى لا يمكن إصدار نسخة ببنك مكسور.
...
gantt
    ...
    ربط فحص المحتوى بالبناء            :p0a, 2026-08-15, 2d
    ذاكرة الأسئلة ومنع التكرار          :p0d, after p0a, 4d
    section عمق ومنحنى (P1)
    توسيع البنوك إلى 40 لكل تصنيف       :p1a, after p0d, 14d
    إظهار المستوى ومنحنى الجولة         :p1b, after p0d, 5d
    نمط التدرّب على الأخطاء             :p1c, after p1a, 6d
