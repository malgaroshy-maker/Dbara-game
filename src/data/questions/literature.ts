import type { TriviaQuestion } from '../../types/quiz';

/** اللغة العربية وأدبها. قواعد الكتابة موثقة في `generalArab.ts`. */
export const literatureQuestions: TriviaQuestion[] = [
  {
    id: 'lit_01',
    category: 'literature',
    difficulty: 'easy',
    question: 'كم عدد حروف الأبجدية العربية؟',
    options: ['26 حرفاً', '29 حرفاً', '28 حرفاً', '32 حرفاً'],
    correctIndex: 2,
    funFact: 'الأبجدية العربية 28 حرفاً، ويُعد بعض اللغويين الهمزة حرفاً مستقلاً فيصبح العدد 29.',
    rewardDinars: 10,
  },
  {
    id: 'lit_02',
    category: 'literature',
    difficulty: 'medium',
    question: 'من هو الشاعر صاحب البيت الشهير: "الخيل والليل والبيداء تعرفني"؟',
    options: ['المتنبي', 'أبو تمام', 'البحتري', 'أبو نواس'],
    correctIndex: 0,
    funFact: 'المتنبي من أعظم شعراء العربية، وبيته هذا من قصيدة في سيف الدولة الحمداني يفخر فيها بشجاعته وشهرته.',
    rewardDinars: 20,
  },
  {
    id: 'lit_03',
    category: 'literature',
    difficulty: 'hard',
    question: 'من هو أول من وضع قواعد علم النحو العربي؟',
    options: ['سيبويه', 'أبو الأسود الدؤلي', 'الخليل بن أحمد', 'ابن مالك'],
    correctIndex: 1,
    funFact: 'وضع أبو الأسود الدؤلي أسس النحو ونقط المصحف، ثم طوره الخليل بن أحمد وسيبويه من بعده.',
    hint: 'وضع النقط على الحروف بأمر من علي بن أبي طالب.',
    rewardDinars: 35,
    source: 'موسوعة المعرفة (مقال: أبو الأسود الدؤلي)',
  },
  {
    id: 'lit_04',
    category: 'literature',
    difficulty: 'easy',
    question: 'من هو صاحب "المقدمة" الذي يُعد مؤسس علم الاجتماع؟',
    options: ['ابن بطوطة', 'ابن خلدون', 'ابن رشد', 'ابن سينا'],
    correctIndex: 1,
    funFact: 'كتب ابن خلدون مقدمته في القرن الرابع عشر، ووضع فيها نظريات في العمران البشري سبقت علم الاجتماع الحديث بقرون.',
    rewardDinars: 10,
  },
  {
    id: 'lit_05',
    category: 'literature',
    difficulty: 'medium',
    question: 'من هو الرحالة المغربي الذي جاب العالم ودوّن رحلته في كتاب "تحفة النظار"؟',
    options: ['ابن جبير', 'الإدريسي', 'ابن بطوطة', 'المسعودي'],
    correctIndex: 2,
    funFact: 'طاف ابن بطوطة نحو 120 ألف كيلومتر عبر أفريقيا وآسيا وأوروبا، ومرّ في رحلته بمدن ليبية منها طرابلس.',
    rewardDinars: 20,
  },
  {
    id: 'lit_06',
    category: 'literature',
    difficulty: 'medium',
    question: 'من هو الشاعر الملقب بـ "أمير الشعراء"؟',
    options: ['حافظ إبراهيم', 'بشارة الخوري', 'إيليا أبو ماضي', 'أحمد شوقي'],
    correctIndex: 3,
    funFact: 'شاعر مصري كبير، بايعه أدباء عصره على لقب يليق بمكانته سنة 1927.',
    rewardDinars: 20,
  },
  {
    id: 'lit_07',
    category: 'literature',
    difficulty: 'easy',
    question: 'من هو الأديب العربي الوحيد الحائز على جائزة نوبل في الأدب؟',
    options: ['طه حسين', 'جبران خليل جبران', 'نجيب محفوظ', 'الطيب صالح'],
    correctIndex: 2,
    funFact: 'نال نجيب محفوظ جائزة نوبل في الأدب سنة 1988، وأشهر أعماله "الثلاثية" و"أولاد حارتنا".',
    rewardDinars: 10,
  },
  {
    id: 'lit_08',
    category: 'literature',
    difficulty: 'expert',
    question: 'من هو صاحب "رسالة الغفران" الملقب برهين المحبسين؟',
    options: ['بديع الزمان الهمذاني', 'الجاحظ', 'ابن المقفع', 'أبو العلاء المعري'],
    correctIndex: 3,
    funFact: 'أبو العلاء المعري شاعر وفيلسوف فقد بصره صغيراً، ولُقب برهين المحبسين: العمى والعزلة.',
    hint: 'حبس نفسه في بيته، وقد فقد بصره صغيراً.',
    rewardDinars: 60,
    source: 'موسوعة المعرفة (مقال: أبو العلاء المعري)',
  },
  {
    id: 'lit_09',
    category: 'literature',
    difficulty: 'hard',
    question: 'من نقل كتاب "كليلة ودمنة" إلى العربية؟',
    options: ['الجاحظ', 'ابن المقفع', 'ابن قتيبة', 'الأصمعي'],
    correctIndex: 1,
    funFact: 'ترجم عبد الله بن المقفع "كليلة ودمنة" عن الفارسية في العصر العباسي، وهو من روائع أدب الحكمة على ألسنة الحيوانات.',
    hint: 'فارسي الأصل، نقل الكتاب عن الفهلوية.',
    rewardDinars: 35,
    source: 'موسوعة المعرفة (مقال: ابن المقفع)',
  },
  {
    id: 'lit_10',
    category: 'literature',
    difficulty: 'hard',
    question: 'من هو صاحب معجم "لسان العرب"؟',
    options: ['الفيروزآبادي', 'الزمخشري', 'الجوهري', 'ابن منظور'],
    correctIndex: 3,
    funFact: '"لسان العرب" لابن منظور من أضخم معاجم العربية، ويضم أكثر من ثمانين ألف مادة لغوية.',
    hint: 'معجمه جمع ما تفرّق في معاجم قبله.',
    rewardDinars: 35,
    source: 'موسوعة المعرفة (مقال: لسان العرب)',
  },
  {
    id: 'lit_11',
    category: 'literature',
    difficulty: 'medium',
    question: 'من هو صاحب المعلقة التي تبدأ بـ "قفا نبكِ من ذكرى حبيبٍ ومنزلِ"؟',
    options: ['عنترة بن شداد', 'طرفة بن العبد', 'زهير بن أبي سلمى', 'امرؤ القيس'],
    correctIndex: 3,
    funFact: 'معلقة امرئ القيس من أشهر المعلقات السبع، وقد لُقب بـ "الملك الضليل" ويُعد من رواد الشعر الجاهلي.',
    rewardDinars: 20,
  },
  {
    id: 'lit_12',
    category: 'literature',
    difficulty: 'medium',
    question: 'ما اسم الأديب الليبي المعروف عالمياً بـ "أدب الصحراء" وصاحب رواية "التبر"؟',
    options: ['خليفة التليسي', 'الصادق النيهوم', 'إبراهيم الكوني', 'أحمد إبراهيم الفقيه'],
    correctIndex: 2,
    funFact: 'روائي ليبي جعل من الصحراء بطلاً لا خلفية، وتُرجمت أعماله إلى لغات كثيرة.',
    rewardDinars: 20,
  },
  {
    id: 'lit_13',
    category: 'literature',
    difficulty: 'expert',
    question: 'بأي لقب يُعرف الشاعر الليبي أحمد رفيق المهدوي؟',
    options: ['شاعر الوطن', 'شاعر النيل', 'شاعر القطرين', 'شاعر الصحراء'],
    correctIndex: 0,
    funFact: 'أُطلق على أحمد رفيق المهدوي لقب "شاعر الوطن" سنة 1960م، وقبلها بعام أفرد له الناقد محمد الصادق عفيفي كتاباً بعنوان "رفيق شاعر الوطنيّة الليبيّة".',
    hint: 'من أشهر شعراء ليبيا في عصر النهضة، ولقبه يجمعه بأرضه.',
    rewardDinars: 60,
    source: 'كتاب "رفيق شاعر الوطنيّة الليبيّة" لمحمد الصادق عفيفي (1959م)، وديوان "شاعر الوطن"',
  },
  {
    id: 'lit_14',
    category: 'literature',
    difficulty: 'easy',
    question: 'ما هو جمع كلمة "كتاب" في اللغة العربية؟',
    options: ['كتابات', 'كُتُب', 'كواتب', 'أكتبة'],
    correctIndex: 1,
    funFact: '"كُتُب" جمع تكسير لكلمة كتاب، وجموع التكسير من خصائص العربية التي تميزها عن كثير من اللغات.',
    rewardDinars: 10,
  },
  {
    id: 'lit_15',
    category: 'literature',
    difficulty: 'medium',
    question: 'ماذا يُسمى الشعر العربي الذي لا يلتزم بالقافية الواحدة ويُكتب بأسطر متفاوتة؟',
    options: ['شعر التفعيلة (الشعر الحر)', 'الشعر العمودي', 'الموشح', 'الزجل'],
    correctIndex: 0,
    funFact: 'ظهر شعر التفعيلة في منتصف القرن العشرين على يد نازك الملائكة وبدر شاكر السياب وغيرهما.',
    rewardDinars: 20,
  },
  {
    id: 'lit_16',
    category: 'literature',
    difficulty: 'hard',
    question: 'من هو مؤسس علم العروض الذي وضع بحور الشعر العربي؟',
    options: ['الخليل بن أحمد الفراهيدي', 'سيبويه', 'الأصمعي', 'ابن جني'],
    correctIndex: 0,
    funFact: 'وضع الخليل بن أحمد الفراهيدي علم العروض وبحور الشعر، كما ألف أول معجم عربي وهو "كتاب العين".',
    hint: 'صاحب معجم العين، وأستاذ سيبويه.',
    rewardDinars: 35,
    source: 'موسوعة المعرفة (مقال: علم العروض)',
  },
  {
    id: 'lit_17',
    category: 'literature',
    difficulty: 'medium',
    question: 'ما معنى كلمة "سُبات" في قوله تعالى: "وجعلنا نومكم سباتاً"؟',
    options: ['سهر طويل', 'تعب شديد', 'نوم عميق', 'قلق دائم'],
    correctIndex: 2,
    funFact: 'السُّبات هو النوم العميق الذي تسكن فيه الحركة، ومنه سُمي "السُّبات" في الطب لحالة فقدان الوعي الطويلة.',
    rewardDinars: 20,
  },
  {
    id: 'lit_18',
    category: 'literature',
    difficulty: 'medium',
    question: 'ماذا تعني كلمة "النوادر" في اللغة العربية؟',
    options: ['قواعد النحو', 'أوزان الشعر', 'خطب المنابر', 'غرائب الكلام وطرائفه'],
    correctIndex: 3,
    funFact: 'أفرد الأدباء العرب كتباً كاملة للنوادر، مثل "البخلاء" للجاحظ الذي جمع طرائف البخل وأخباره.',
    rewardDinars: 20,
  },
  {
    id: 'lit_19',
    category: 'literature',
    difficulty: 'hard',
    question: 'ما مرادف كلمة "المِنّة" في اللغة؟',
    options: ['الإحسان والإنعام', 'الشدة والقسوة', 'الحيرة والتردد', 'السفر الطويل'],
    correctIndex: 0,
    funFact: 'المنّة هي العطاء والإحسان، ومنها قوله تعالى "بل الله يمن عليكم أن هداكم للإيمان".',
    hint: 'من معانيها العطاء بلا مقابل، وقد تُذكر في سياق التفضّل.',
    rewardDinars: 35,
    source: 'ويكيبيديا الموسوعة الحرة (مقال: المنان (أسماء الله الحسنى))',
  },
  {
    id: 'lit_20',
    category: 'literature',
    difficulty: 'medium',
    question: 'ما معنى كلمة "فَوج"؟',
    options: ['إناء لحفظ الماء', 'جماعة من الناس تأتي معاً', 'طريق ضيق بين جبلين', 'نوع من السحاب'],
    correctIndex: 1,
    funFact: 'ومنها قوله تعالى "ورأيت الناس يدخلون في دين الله أفواجاً"، أي جماعات متتابعة.',
    rewardDinars: 20,
  },
  {
    id: 'lit_21',
    category: 'literature',
    difficulty: 'hard',
    question: 'ما اسم الكيس الصغير الذي كان العرب يشدون فيه نقودهم؟',
    options: ['الجراب', 'المزود', 'الصرة', 'الخرج'],
    correctIndex: 2,
    funFact: 'الصُّرّة كيس صغير تُشد فيه الدراهم، ومنه قولهم "صرّ الدراهم" أي جمعها وأحكم ربطها.',
    hint: 'يُشدّ من أعلى، ومنه جاء التعبير «صرّ النقود».',
    rewardDinars: 35,
    source: 'ويكيبيديا الموسوعة الحرة (مقال: صرة نقود)',
  },
  {
    id: 'lit_22',
    category: 'literature',
    difficulty: 'easy',
    question: 'ما مرادف كلمة "الغيم"؟',
    options: ['المطر', 'الضباب', 'الندى', 'السحاب'],
    correctIndex: 3,
    funFact: 'للسحاب في العربية أسماء كثيرة بحسب حاله: الغيم والغمام والمزن والعارض والركام.',
    rewardDinars: 10,
  },
  {
    id: 'lit_23',
    category: 'literature',
    difficulty: 'medium',
    question: 'ماذا تعني كلمة "النَّهب"؟',
    options: ['أخذ المال جهراً وقهراً', 'الادخار للمستقبل', 'الإقراض بفائدة', 'التبرع للفقراء'],
    correctIndex: 0,
    funFact: 'تفرّق العربية بين النهب وهو الأخذ جهاراً، والسرقة وهي الأخذ خفية، والاختلاس وهو الأخذ على غفلة.',
    rewardDinars: 20,
  },
  {
    id: 'lit_24',
    category: 'literature',
    difficulty: 'easy',
    question: 'إلى كم قسم ينقسم الكلام في اللغة العربية؟',
    options: ['ثلاثة أقسام', 'قسمان', 'أربعة أقسام', 'خمسة أقسام'],
    correctIndex: 0,
    funFact: 'أقسام الكلام ثلاثة: اسم وفعل وحرف، وعلى هذا التقسيم بُني النحو العربي كله.',
    rewardDinars: 10,
  },
  {
    id: 'lit_25',
    category: 'literature',
    difficulty: 'easy',
    question: 'ما ضد كلمة "الظلام" في اللغة العربية؟',
    options: ['البرد', 'النور', 'الصمت', 'البعد'],
    correctIndex: 1,
    funFact: 'التضاد باب واسع في العربية، وقد أفرد له اللغويون كتباً في الأضداد ومعانيها.',
    rewardDinars: 10,
  },
  {
    id: 'lit_26',
    category: 'literature',
    difficulty: 'easy',
    question: 'ما جمع كلمة "قلم"؟',
    options: ['قلمات', 'قوالم', 'أقلام', 'قلمون'],
    correctIndex: 2,
    funFact: '"أقلام" جمع تكسير، وقد ورد في القرآن الكريم في قوله تعالى "ولو أنما في الأرض من شجرة أقلام".',
    rewardDinars: 10,
  },
  {
    id: 'lit_27',
    category: 'literature',
    difficulty: 'medium',
    question: 'ماذا تُسمى القصائد الجاهلية السبع الأشهر التي قيل إنها عُلقت على الكعبة؟',
    options: ['المفضليات', 'الحماسة', 'المعلقات', 'المقامات'],
    correctIndex: 2,
    funFact: 'أشهر أصحاب المعلقات امرؤ القيس وعنترة وزهير وطرفة، وهي من أنفس ما وصلنا من الشعر الجاهلي.',
    rewardDinars: 20,
  },
  {
    id: 'lit_28',
    category: 'literature',
    difficulty: 'medium',
    question: 'ما اسم العلم الذي يبحث في أوزان الشعر العربي وموسيقاه؟',
    options: ['البلاغة', 'العروض', 'الصرف', 'النحو'],
    correctIndex: 1,
    funFact: 'بحور الشعر العربي ستة عشر بحراً، لكل بحر تفعيلات ثابتة يُقاس عليها البيت.',
    rewardDinars: 20,
  },
  {
    id: 'lit_29',
    category: 'literature',
    difficulty: 'medium',
    question: 'من الشاعر الملقب بـ "شاعر النيل"؟',
    options: ['أحمد شوقي', 'حافظ إبراهيم', 'إيليا أبو ماضي', 'خليل مطران'],
    correctIndex: 1,
    funFact: 'لقب حمله شاعر مصري كبير، بايعه أدباء عصره عليه سنة 1927.',
    rewardDinars: 20,
  },
  {
    id: 'lit_30',
    category: 'literature',
    difficulty: 'medium',
    question: 'ما معنى كلمة "الوجيز"؟',
    options: ['الطويل المسهب', 'الغامض', 'المختصر الموجز', 'المكرر'],
    correctIndex: 2,
    funFact: 'الإيجاز من أبواب البلاغة، ومنه قولهم "خير الكلام ما قلّ ودلّ".',
    rewardDinars: 20,
  },
  {
    id: 'lit_31',
    category: 'literature',
    difficulty: 'medium',
    question: 'ماذا يُسمى تشبيه الشيء بغيره مع حذف أداة التشبيه والمشبّه به الصريح؟',
    options: ['الكناية', 'الطباق', 'السجع', 'الاستعارة'],
    correctIndex: 3,
    funFact: 'الاستعارة من أبرز صور البيان، وتقوم على نقل اللفظ من معناه الأصلي لعلاقة المشابهة.',
    rewardDinars: 20,
  },
  {
    id: 'lit_32',
    category: 'literature',
    difficulty: 'hard',
    question: 'من مؤلف كتاب "البخلاء"؟',
    options: ['الجاحظ', 'ابن قتيبة', 'التوحيدي', 'الأصمعي'],
    correctIndex: 0,
    funFact: 'جمع الجاحظ في "البخلاء" طرائف البخل وأخباره، وهو من أمتع كتب النثر في العصر العباسي.',
    hint: 'بصري، وله كتاب الحيوان أيضاً.',
    source: 'ويكيبيديا العربية (مقال: البخلاء)',
    rewardDinars: 35,
  },
  {
    id: 'lit_33',
    category: 'literature',
    difficulty: 'hard',
    question: 'من صاحب "المقامات" الذي ابتكر هذا الفن النثري في القرن الرابع الهجري؟',
    options: ['الحريري', 'بديع الزمان الهمذاني', 'ابن المقفع', 'الخوارزمي'],
    correctIndex: 1,
    funFact: 'ابتكر بديع الزمان الهمذاني فن المقامة، ثم بلغ به الحريري ذروة الصنعة اللفظية بعده بقرن.',
    hint: 'لقبه يصف تفرده في زمانه.',
    source: 'ويكيبيديا العربية (مقال: مقامات بديع الزمان الهمذاني)',
    rewardDinars: 35,
  },
  {
    id: 'lit_34',
    category: 'literature',
    difficulty: 'hard',
    question: 'ما اسم الشاعر العذري الذي اشتهر بحبه لليلى؟',
    options: ['جميل بثينة', 'كثير عزة', 'قيس بن الملوح', 'عروة بن حزام'],
    correctIndex: 2,
    funFact: 'عُرف قيس بن الملوح بـ "مجنون ليلى"، وصار قصته مادة للشعر والمسرح في آداب كثيرة.',
    hint: 'اشتهر بلقب يصف ما بلغه من الحب.',
    source: 'ويكيبيديا العربية (مقال: قيس بن الملوح)',
    rewardDinars: 35,
  },
  {
    id: 'lit_35',
    category: 'literature',
    difficulty: 'hard',
    question: 'ما أكثر بحور الشعر استعمالاً عند شعراء الجاهلية؟',
    options: ['الرجز', 'الكامل', 'الوافر', 'الطويل'],
    correctIndex: 3,
    funFact: 'نُظمت أكثر المعلقات على البحر الطويل، لسعته التي تحتمل المعاني الطويلة والوصف المفصل.',
    hint: 'اسم البحر يصف عدد تفعيلاته.',
    source: 'موسوعة موضوع (مقال: بحر الطويل في الشعر العربي)',
    rewardDinars: 35,
  },
  {
    id: 'lit_36',
    category: 'literature',
    difficulty: 'hard',
    question: 'ما اسم فن الشعر الذي ابتكره الأندلسيون متحرراً من نظام القصيدة العمودية؟',
    options: ['الموشح', 'المقامة', 'الرثاء', 'المديح'],
    correctIndex: 0,
    funFact: 'الموشح مقسم إلى أدوار وأقفال بقواف متعددة، ونشأ في الأندلس ليُغنّى لا ليُلقى فحسب.',
    hint: 'نشأ في الأندلس، ووُضع ليُغنّى.',
    source: 'ويكيبيديا العربية (مقال: موشح أندلسي)',
    rewardDinars: 35,
  },
  {
    id: 'lit_37',
    category: 'literature',
    difficulty: 'hard',
    question: 'ما معنى كلمة "الفلاة"؟',
    options: ['قمة الجبل', 'الصحراء الواسعة الخالية', 'مجرى النهر', 'الغابة الكثيفة'],
    correctIndex: 1,
    funFact: 'الفلاة من ألفاظ البادية الكثيرة في العربية، ومثلها البيداء والمهمه والقفر.',
    hint: 'مكان لا ماء فيه ولا أنيس.',
    source: 'المعجم الوسيط (مجمع اللغة العربية بالقاهرة، مادة ف ل و)',
    rewardDinars: 35,
  },
  {
    id: 'lit_38',
    category: 'literature',
    difficulty: 'expert',
    question: 'من مؤلف كتاب "الأغاني" الذي جمع أخبار الشعراء والمغنين؟',
    options: ['ابن خلكان', 'أبو الفرج الأصفهاني', 'الأصمعي', 'ابن سلام الجمحي'],
    correctIndex: 1,
    funFact: 'يُعد "الأغاني" موسوعة للأدب والتاريخ الاجتماعي، وقد استغرق تأليفه خمسين سنة.',
    hint: 'أموي النسب رغم أنه عاش في بغداد العباسية.',
    rewardDinars: 60,
    source: 'كتاب الأغاني لأبي الفرج الأصفهاني',
  },
  {
    id: 'lit_39',
    category: 'literature',
    difficulty: 'expert',
    question: 'ما معنى كلمة "الطِّرس"؟',
    options: ['قلم من قصب', 'حبر أسود', 'صحيفة مُحيت ثم كُتب فيها', 'خزانة الكتب'],
    correctIndex: 2,
    funFact: 'كان الطِّرس حلاً لغلاء الورق قديماً، إذ تُمحى الكتابة القديمة ويُعاد استعمال الرَّق.',
    hint: 'منه جاء وصف الورق المُعاد استعماله بعد محو ما فيه.',
    source: 'ويكيبيديا العربية (مقال: طرس)',
    rewardDinars: 60,
  },
  {
    id: 'lit_40',
    category: 'literature',
    difficulty: 'expert',
    question: 'ما اسم المعجم الذي ألفه الفيروزآبادي؟',
    options: ['الصحاح', 'أساس البلاغة', 'المخصص', 'القاموس المحيط'],
    correctIndex: 3,
    funFact: 'ذاع "القاموس المحيط" حتى صارت كلمة "قاموس" نفسها تعني المعجم في الاستعمال العربي الحديث.',
    hint: 'معجم رتّبه صاحبه على الأبواب والفصول، واختصر به ما قبله.',
    rewardDinars: 60,
    source: 'معجم القاموس المحيط لمجد الدين الفيروزآبادي',
  },
  {
    id: 'lit_mis_02',
    category: 'literature',
    cityId: 'misrata',
    difficulty: 'medium',
    question: 'من هو الأديب والمؤرخ الليبي البارز، مؤلف \'أعلام من طرابلس\' و\'صحافة ليبيا في نصف قرن\'، والذي نُسبت كنيته إلى مدينة مصراتة؟',
    options: ['علي مصطفى المصراتي', 'خليفة التليسي', 'علي فهمي خشيم', 'أحمد رفيق المهدوي'],
    correctIndex: 0,
    funFact: 'تولى المصراتي رئاسة أمانة اتحاد الأدباء والكتاب في ليبيا وتُرجمت أعماله وقصصه إلى عدة لغات عالمية.',
    hint: 'أديب وصحفي ومؤرخ رائد ينتهي لقبه باسم مدينة مصراتة.',
    rewardDinars: 20,
    source: 'موسوعة ويكيبيديا العربية (مادة: علي مصطفى المصراتي)',
  },
  {
    id: 'lit_der_02',
    category: 'literature',
    cityId: 'derna',
    difficulty: 'hard',
    question: 'من هو الشاعر والقاضي الوطني الليبي الرائد، ابن مدينة درنة، وصاحب ديوان \'البلبل والوكر\' وقصيدة \'صحوة الضمير الإنساني\'؟',
    options: ['إبراهيم الأسطى عمر', 'أحمد رفيق المهدوي', 'علي الرقيعي', 'أحمد الشارف'],
    correctIndex: 0,
    funFact: 'نال إبراهيم الأسطى عمر شهرة واسعة بقصائده الإنسانية والدعوة لنبذ النزاعات والدمار في أربعينيات القرن الماضي وانتُخب نائباً في البرلمان.',
    hint: 'شاعر وطني من أعلام درنة جمع بين سلك القضاء ونظم الشعر والعمل في جمعية عمر المختار.',
    rewardDinars: 35,
    source: 'موسوعة ويكيبيديا العربية (مادة: إبراهيم الأسطى عمر)',
  },
  {
    id: 'lit_zaw_02',
    category: 'literature',
    cityId: 'zawiya',
    difficulty: 'medium',
    question: 'من هو العالم والمؤرخ الليبي الكبير، مفتي الديار الليبية الأسبق ومؤلف \'معجم البلدان الليبية\' و\'أعلام ليبيا\'، الذي وُلد في قرية الحرشة بالزاوية؟',
    options: ['الطاهر أحمد الزاوي', 'علي مصطفى المصراتي', 'عبد السلام الأسمر', 'محمد بن علي السنوسي'],
    correctIndex: 0,
    funFact: 'يعد كتاب \'معجم البلدان الليبية\' للطاهر الزاوي المرجع الجغرافي والتاريخي الأوسع لتوثيق أسماء مدن وقرى وجبال ليبيا.',
    hint: 'عالم لغوي ومؤرخ ومفتٍ يحمل لقبه اسم مدينة الزاوية.',
    rewardDinars: 20,
    source: 'موسوعة ويكيبيديا العربية (مادة: الطاهر الزاوي)',
  },
];
