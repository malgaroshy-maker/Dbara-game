import type { TriviaQuestion } from '../../types/quiz';

/** اللهجة الليبية والأمثال الشعبية. قواعد الكتابة موثقة في `generalArab.ts`. */
export const dialectQuestions: TriviaQuestion[] = [
  {
    id: 'dia_01',
    category: 'dialects',
    difficulty: 'easy',
    question: 'في اللهجة الليبية، ماذا تعني عبارة "الجو صقع"؟',
    options: ['الجو بارد جداً', 'الجو حار جداً', 'الجو ممطر', 'الجو مغبر'],
    correctIndex: 0,
    funFact: '"صقع" مأخوذة من الصقيع، وتُقال عند اشتداد البرد في الشتاء الليبي وخاصة في الجبل والمناطق الداخلية.',
    rewardDinars: 10,
  },
  {
    id: 'dia_02',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ما معنى الكلمة الليبية الشهيرة "هلبة"؟',
    options: ['كثير جداً', 'سريع', 'قليل ونادر', 'متأخر'],
    correctIndex: 0,
    funFact: '"هلبة" من أكثر الكلمات استخداماً في ليبيا، وتعني الكثير أو الوفير، وتُستعمل للمدح والتعجب.',
    rewardDinars: 10,
  },
  {
    id: 'dia_13',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ماذا تعني كلمة "توّا" في اللهجة الليبية؟',
    options: ['أمس', 'غداً', 'الآن', 'ربما'],
    correctIndex: 2,
    funFact: '"توّا" تعني الآن أو حالاً، وتُستخدم كذلك بمعنى "بعد قليل" حسب سياق الجملة.',
    rewardDinars: 10,
  },
  {
    id: 'dia_14',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ماذا يقصد الليبي بكلمة "باهي"؟',
    options: ['متعب', 'غالي الثمن', 'حسناً وجيد', 'بعيد'],
    correctIndex: 2,
    funFact: '"باهي" من أكثر كلمات الموافقة شيوعاً في ليبيا، وتقابل "حسناً" أو "تمام" في بقية اللهجات العربية.',
    rewardDinars: 10,
  },
  {
    id: 'dia_15',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ماذا تعني كلمة "نجّم" في الجملة الليبية "ما نجّمش نجي"؟',
    options: ['أرغب', 'أعرف', 'أتذكر', 'أستطيع'],
    correctIndex: 3,
    funFact: '"نجّم" تعني استطاع وقدر، و"ما نجّمش" أي لا أستطيع، وهي شائعة في ليبيا وتونس.',
    rewardDinars: 20,
  },
  {
    id: 'dia_03',
    category: 'dialects',
    difficulty: 'medium',
    question: 'أكمل المثل الشعبي الليبي: "اللي ما يعرفك..."؟',
    options: ['يخسرك', 'يجهلك', 'يعذرك', 'يسألك'],
    correctIndex: 1,
    funFact: 'مثل شائع يعني أن من لا يعرف قدرك الحقيقي قد يسيء تقديرك، فلا تحمل همّ جهله بك.',
    rewardDinars: 20,
  },
  {
    id: 'dia_05',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ماذا يقصد الليبيون عندما يصفون شخصاً بأنه "فالح"؟',
    options: ['كثير النوم', 'سريع الغضب', 'شاطر وماهر ومجتهد', 'كثير السفر'],
    correctIndex: 2,
    funFact: '"فالح" في اللهجة الليبية والمغاربية تدل على النجابة والمهارة والهمة في العمل.',
    rewardDinars: 20,
  },
  {
    id: 'dia_06',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ماذا تعني كلمة "دوة" في العبارة الليبية "بلاش دوة زايدة"؟',
    options: ['حركة سريعة', 'كلام وحديث', 'ضجة وصياح', 'مصاريف كثيرة'],
    correctIndex: 1,
    funFact: '"الدوة" هي الحديث، ويقال "يدوي" أي يتكلم، و"دوّاي" للشخص كثير الكلام.',
    rewardDinars: 20,
  },
  {
    id: 'dia_08',
    category: 'dialects',
    difficulty: 'easy',
    question: 'في اللهجة الليبية، ماذا تعني كلمة "سمح"؟',
    options: ['جميل وحسن', 'قبيح', 'طويل القامة', 'قديم بالٍ'],
    correctIndex: 0,
    funFact: '"سمح" من السماحة وحسن الهيئة، وتكثر في برقة وفزان، ويقابلها "زوين" في مناطق أخرى.',
    rewardDinars: 10,
  },
  {
    id: 'dia_09',
    category: 'dialects',
    difficulty: 'medium',
    question: 'أكمل المثل المتداول في ليبيا: "ما طار طير وارتفع إلا كما طار..."؟',
    options: ['ابتعد', 'نزل', 'وقع', 'ضاع'],
    correctIndex: 2,
    funFact: 'حكمة عربية متداولة تدعو للتواضع وتحذر من الغرور مهما علا شأن الإنسان.',
    rewardDinars: 20,
  },
  {
    id: 'dia_10',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ماذا يقصد الليبي عندما يقول: "دير بالك"؟',
    options: ['انتبه واحذر', 'أسرع في المشي', 'اذهب للنوم', 'تكلم بصوت عالٍ'],
    correctIndex: 0,
    funFact: '"دير بالك" أي انتبه وركّز، وغالباً ما تُسبق بنداء التحبب "يا ودي".',
    rewardDinars: 10,
  },
  {
    id: 'dia_11',
    category: 'dialects',
    difficulty: 'expert',
    question: 'أكمل المثل الشعبي المتداول: "من لحيته..."؟',
    options: ['قصّ شواربه', 'أطعمه عسل', 'علمه الصبر', 'افتّله حبل'],
    correctIndex: 3,
    funFact: 'صورته أن الفتل جاء من شعر لحيته هو، فلم يحتج الأمر شيئاً من خارجه؛ ويُضرب لمن تُقضى حاجته من ماله ومورده دون كلفة على من قضاها.',
    hint: 'يُضرب فيمن يُؤخذ من عنده ما يُوثَق به عليه.',
    source: 'ويكي مصدر — الأمثال العامية لأحمد تيمور باشا، مثل رقم 2817: «من دقنه فتلوا له حبل»، ويُروى «من دقنه افتل له»',
    rewardDinars: 60,
  },
  {
    id: 'dia_12',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ماذا تعني التحية الليبية اليومية "شن الجو؟"',
    options: ['ما هو الطقس غداً؟', 'كيف الحال والأخبار؟', 'هل أنت مسافر؟', 'كم الساعة الآن؟'],
    correctIndex: 1,
    funFact: '"شن الجو" أشهر تحية شبابية ليبية، وهي سؤال عن الأحوال والأخبار لا عن الطقس.',
    rewardDinars: 10,
  },
  {
    id: 'dia_16',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ماذا تعني كلمة "كرهبة" في اللهجة الليبية؟',
    options: ['الدراجة', 'السيارة', 'الحقيبة', 'النافذة'],
    correctIndex: 1,
    funFact: '"كرهبة" من أشهر الكلمات الليبية، ويرى كثيرون أنها مرتبطة بلفظ "الكهرباء" في أصل تسميتها.',
    rewardDinars: 10,
  },
  {
    id: 'dia_17',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ماذا يقصد الليبي بكلمة "حوش"؟',
    options: ['السوق', 'المدرسة', 'الحديقة العامة', 'البيت والدار'],
    correctIndex: 3,
    funFact: '"الحوش" في ليبيا هو البيت كله لا فناؤه فقط، ومنها "حوش العايلة" أي بيت الأسرة.',
    rewardDinars: 10,
  },
  {
    id: 'dia_18',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ما معنى كلمة "شنو" في اللهجة الليبية؟',
    options: ['ماذا', 'أين', 'متى', 'كيف'],
    correctIndex: 0,
    funFact: '"شنو" أداة استفهام بمعنى "ماذا"، ومنها "شن" و"شنهو" في مناطق مختلفة من ليبيا.',
    rewardDinars: 10,
  },
  {
    id: 'dia_19',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ماذا تعني كلمة "قداش" في اللهجة الليبية؟',
    options: ['لماذا', 'كم', 'من', 'أين'],
    correctIndex: 1,
    funFact: '"قداش" أداة استفهام عن الكم أو السعر، ومنها السؤال الشهير في السوق: "قداش هذا؟"',
    rewardDinars: 20,
  },
  {
    id: 'dia_20',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ما المقصود بكلمة "العشية" في الحديث اليومي الليبي؟',
    options: ['وقت الفجر', 'فترة ما بعد الظهر والمساء', 'منتصف الليل', 'وقت الظهيرة'],
    correctIndex: 1,
    funFact: '"العشية" هي فترة ما بعد الظهر حتى المغرب، وهي وقت الزيارات وجلسات الشاي في العرف الليبي.',
    rewardDinars: 20,
  },
  {
    id: 'dia_21',
    category: 'dialects',
    difficulty: 'medium',
    question: 'أكمل المثل المتداول في ليبيا: "يد وحدة ما..."؟',
    options: ['تبني دار', 'تحمل حملين', 'تصفق', 'تكتب'],
    correctIndex: 2,
    funFact: 'مثل يحث على التعاون والعمل الجماعي، فالفرد وحده لا ينجز ما ينجزه المجتمع المتكاتف.',
    rewardDinars: 20,
  },
  {
    id: 'dia_22',
    category: 'dialects',
    difficulty: 'hard',
    question: 'ماذا تعني كلمة "زعمة" في السؤال الليبي "زعمة جا ولا لا؟"',
    options: ['يا ترى / هل', 'بالتأكيد', 'أبداً', 'ربما لاحقاً'],
    correctIndex: 0,
    funFact: '"زعمة" أداة استفهام وتعجب بمعنى "يا ترى"، وتُستخدم للاستفسار والتشكيك في آن واحد.',
    hint: 'أداة استفهام تُبدأ بها الجملة، لا اسم ولا فعل.',
    source: 'معجم اللهجات المحكية (موقع معجم)',
    rewardDinars: 35,
  },
  {
    id: 'dia_23',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ماذا تعني كلمة "قطّوس" في اللهجة الليبية؟',
    options: ['القط', 'الكلب', 'العصفور', 'الفأر'],
    correctIndex: 0,
    funFact: '"قطّوس" للذكر و"قطّوسة" للأنثى، وهي من المفردات المشتركة بين ليبيا وبلدان المغرب العربي.',
    rewardDinars: 10,
  },
  {
    id: 'dia_24',
    category: 'dialects',
    difficulty: 'easy',
    question: 'ماذا يقصد الليبي بكلمة "برّا"؟',
    options: ['ادخل', 'اخرج أو في الخارج', 'اجلس', 'انتظر'],
    correctIndex: 1,
    funFact: '"برّا" تعني الخارج، ويقابلها "لداخل"، ومنها قولهم "قاعد برّا" أي جالس في الخارج.',
    rewardDinars: 10,
  },
  {
    id: 'dia_25',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ماذا يسأل الليبي حين يقول "شبيك؟"',
    options: ['أين أنت؟', 'متى وصلت؟', 'ما بك؟ ماذا أصابك؟', 'كم عمرك؟'],
    correctIndex: 2,
    funFact: '"شبيك" اختصار لـ "ما شا بك"، وتُقال للاطمئنان على من بدا عليه تغيّر أو انزعاج.',
    rewardDinars: 20,
  },
  {
    id: 'dia_26',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ما معنى كلمة "علاش" في اللهجة الليبية؟',
    options: ['كيف', 'متى', 'أين', 'لماذا'],
    correctIndex: 3,
    funFact: '"علاش" مركبة من "على" و"أش"، ومثلها "كيفاش" بمعنى كيف، و"قداش" بمعنى كم.',
    rewardDinars: 20,
  },
  {
    id: 'dia_27',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ماذا تعني كلمة "وين" في اللهجة الليبية؟',
    options: ['أين', 'لماذا', 'كيف', 'ماذا'],
    correctIndex: 0,
    funFact: '"وين" أداة استفهام عن المكان، ومنها السؤال اليومي "وين رايح؟" أي إلى أين تذهب.',
    rewardDinars: 20,
  },
  {
    id: 'dia_28',
    category: 'dialects',
    difficulty: 'medium',
    question: 'بماذا يصف الليبي الطعام حين يقول عنه "بنّين"؟',
    options: ['بارد', 'لذيذ الطعم', 'كثير الملح', 'غالي الثمن'],
    correctIndex: 1,
    funFact: '"بنّين" من "البنّة" أي الطعم الطيب، ويقال "بنّتها حلوة" للأكلة المتقنة.',
    rewardDinars: 20,
  },
  {
    id: 'dia_29',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ماذا تعني كلمة "زربة" في العبارة الليبية "علاش الزربة؟"',
    options: ['الضحك', 'الغضب', 'العجلة والاستعجال', 'التعب'],
    correctIndex: 2,
    funFact: '"زربة" أي عجلة، ويقال "لا تزرب" أي لا تتعجل، و"مزروب" للمستعجل.',
    rewardDinars: 20,
  },
  {
    id: 'dia_30',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ماذا تعني كلمة "هني" في اللهجة الليبية؟',
    options: ['هناك', 'أمس', 'غداً', 'هنا'],
    correctIndex: 3,
    funFact: '"هني" للمكان القريب و"غادي" للبعيد، ومنها "تعال هني" أي تعال إلى هنا.',
    rewardDinars: 20,
  },
  {
    id: 'dia_31',
    category: 'dialects',
    difficulty: 'medium',
    question: 'ماذا تعني كلمة "امبعد" في الحديث الليبي؟',
    options: ['بعد ذلك أو لاحقاً', 'قبل قليل', 'دائماً', 'أبداً'],
    correctIndex: 0,
    funFact: '"امبعد" ظرف زمان يعني ما يأتي لاحقاً، وتُستعمل أيضاً للاستفهام بمعنى "وماذا بعد؟"',
    rewardDinars: 20,
  },
  {
    id: 'dia_32',
    category: 'dialects',
    difficulty: 'hard',
    question: 'ما هي "الدبّوزة" في اللهجة الليبية؟',
    options: ['الملعقة', 'القارورة أو الزجاجة', 'الطاولة', 'المفتاح'],
    correctIndex: 1,
    funFact: '"الدبّوزة" هي القارورة، ويرجّح أنها دخيلة على اللهجة من الإيطالية، شأنها شأن مفردات ساحلية أخرى.',
    hint: 'الكلمة مأخوذة عن الإيطالية، وتُقال لما يُحفظ فيه الماء.',
    source: 'معجم اللهجات المحكية (موقع معجم)',
    rewardDinars: 35,
  },
  {
    id: 'dia_33',
    category: 'dialects',
    difficulty: 'hard',
    question: 'ما هي "الفلوكة" في لهجة أهل الساحل الليبي؟',
    options: ['الشبكة', 'المجداف', 'القارب الصغير', 'المرساة'],
    correctIndex: 2,
    funFact: 'الفلوكة قارب خشبي صغير يستعمله الصيادون قرب الشاطئ، وتكثر في موانئ زوارة ومصراتة وطبرق.',
    hint: 'الكلمة من أصل إيطالي، ويستعملها الصيادون كل يوم.',
    source: 'المعجم الوسيط (مجمع اللغة العربية بالقاهرة، مادة ف ل ك)',
    rewardDinars: 35,
  },
  {
    id: 'dia_34',
    category: 'dialects',
    difficulty: 'hard',
    question: 'ماذا تعني كلمة "زنقة" في اللهجة الليبية؟',
    options: ['الساحة الواسعة', 'السوق', 'البوابة', 'الشارع الضيق'],
    correctIndex: 3,
    funFact: '"الزنقة" ممر ضيق بين البيوت، وتكثر في نسيج المدن القديمة كالمدينة القديمة بطرابلس.',
    hint: 'تُقال للطريق الضيق بين البيوت في المدينة القديمة.',
    source: 'لسان العرب لابن منظور (مادة ز ن ق)',
    rewardDinars: 35,
  },
  {
    id: 'dia_35',
    category: 'dialects',
    difficulty: 'hard',
    question: 'ما هي "المقشّة" في البيت الليبي؟',
    options: ['المكنسة', 'المرآة', 'السلّم', 'الوسادة'],
    correctIndex: 0,
    funFact: '"المقشّة" من القش الذي كانت تُصنع منه قديماً، ومنها الفعل "قشّ" أي كنس.',
    hint: 'أداة تُمسك باليد وتُجرّ على الأرض.',
    source: 'ويكيبيديا العربية (مقال: مكنسة)',
    rewardDinars: 35,
  },
  {
    id: 'dia_36',
    category: 'dialects',
    difficulty: 'hard',
    question: 'ما هو "الكانون" في البيت الليبي التقليدي؟',
    options: ['خزانة الملابس', 'موقد الفحم', 'باب الدار', 'سطح المنزل'],
    correctIndex: 1,
    funFact: 'على الكانون يُغلى الشاي ويُشوى الخبز في ليالي الشتاء، وحوله تلتئم جلسة العائلة.',
    hint: 'يُوقد فيه الفحم للتدفئة ولغلي الشاي.',
    source: 'ويكيبيديا العربية (مقال: كانون (موقد))',
    rewardDinars: 35,
  },
  {
    id: 'dia_37',
    category: 'dialects',
    difficulty: 'hard',
    question: 'ما هي "الشكارة" في الاستعمال الليبي؟',
    options: ['الحبل', 'السكين', 'الكيس الكبير', 'العصا'],
    correctIndex: 2,
    funFact: '"الشكارة" كيس كبير من الخيش أو البلاستيك يُعبأ فيه الدقيق أو الحبوب أو التمر.',
    hint: 'يُحمل فيه الدقيق أو الحبوب.',
    source: 'تكملة المعاجم العربية لرينهارت دوزي (مادة شكر)',
    rewardDinars: 35,
  },
  {
    id: 'dia_38',
    category: 'dialects',
    difficulty: 'hard',
    question: 'ما هي "الزريبة" في الريف الليبي؟',
    options: ['البئر', 'الطاحونة', 'المخزن', 'حظيرة الماشية'],
    correctIndex: 3,
    funFact: 'الزريبة سياج من الشوك أو الخشب يُحاط به الغنم والماعز ليلاً حمايةً لها.',
    hint: 'مكان مسوّر يُحفظ فيه الغنم.',
    source: 'ويكيبيديا العربية (مقال: حظيرة)',
    rewardDinars: 35,
  },
  {
    id: 'dia_39',
    category: 'dialects',
    difficulty: 'hard',
    question: 'ما هو "الطبسي" على المائدة الليبية؟',
    options: ['الصحن الكبير', 'الكوب', 'الشوكة', 'الإبريق'],
    correctIndex: 0,
    funFact: 'يُقدَّم البازين والفتات في طبسي كبير يجتمع حوله الآكلون، وهي عادة راسخة في الضيافة الليبية.',
    hint: 'يُوضع في وسط المائدة ويأكل منه الجميع.',
    source: 'تكملة المعاجم العربية لرينهارت دوزي (مادة طبس)',
    rewardDinars: 35,
  },
  {
    id: 'dia_40',
    category: 'dialects',
    difficulty: 'expert',
    question: 'ماذا يقصد الليبي بكلمة "الحوت"؟',
    options: ['الحوت العملاق', 'السمك عموماً', 'الطُّعم', 'قارب الصيد'],
    correctIndex: 1,
    funFact: 'خلافاً للفصحى، يعني "الحوت" في ليبيا والمغرب العربي السمك بأنواعه، ومنه "حوت مشوي".',
    hint: 'الاسم عند أهل الساحل يشمل كل ما يخرج من البحر، لا نوعاً بعينه.',
    source: 'لسان العرب لابن منظور (مادة ح و ت)',
    rewardDinars: 60,
  },
  {
    id: 'dia_41',
    category: 'dialects',
    difficulty: 'expert',
    question: 'ماذا تعني كلمة "ديمة" في اللهجة الليبية؟',
    options: ['أحياناً', 'نادراً', 'دائماً', 'غداً'],
    correctIndex: 2,
    funFact: '"ديمة" من الدوام، ويقال "ديمة كذا" أي هذا حاله على الدوام.',
    hint: 'ضدها «مرة مرة»، وتُقال لمن لا يتغير حاله.',
    source: 'معجم اللهجات المحكية (موقع معجم)',
    rewardDinars: 60,
  },
  {
    id: 'dia_42',
    category: 'dialects',
    difficulty: 'expert',
    question: 'ما هو "الكسكاس" في المطبخ الليبي؟',
    options: ['لوح تقطيع الخضار', 'مغرفة خشبية', 'إبريق الشاي', 'إناء مثقوب لطهي الكسكسي بالبخار'],
    correctIndex: 3,
    funFact: 'يوضع الكسكاس فوق قدر المرق فيصعد البخار من ثقوبه لينضج الكسكسي دون أن يلامس الماء.',
    hint: 'قطعة من عدة البخار، تُوضع فوق القدر لا فيها.',
    source: 'ويكيبيديا العربية (مقال: كسكاس)',
    rewardDinars: 60,
  },
  {
    id: 'dia_jal_02',
    category: 'dialects',
    cityId: 'jalu_awjila',
    difficulty: 'hard',
    question: 'ما اسم اللغة المحلية الفريدة التي حافظ عليها سكان واحة أوجلة عبر القرون إلى جانب العربية؟',
    options: ['اللغة الأوجلية', 'اللغة التارقية', 'اللغة الغدامسية', 'اللغة السيوية'],
    correctIndex: 0,
    funFact: 'من أقدم الألسنة الحية في الواحات الشرقية، واحتفاظها بخصائص صوتية قديمة لفت اهتمام علماء اللسانيات.',
    hint: 'لغة شرقية مشتقة مباشرة من اسم الواحة التاريخية ذاتها.',
    rewardDinars: 35,
    source: 'موسوعة ويكيبيديا العربية (مادة: اللغة الأوجلية)',
  },
];
