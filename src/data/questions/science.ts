import type { TriviaQuestion } from '../../types/quiz';

/** العلوم والطبيعة. قواعد الكتابة موثقة في `generalArab.ts`. */
export const scienceQuestions: TriviaQuestion[] = [
  {
    id: 'sci_01',
    category: 'science',
    difficulty: 'easy',
    question: 'ما هو الكوكب الأقرب إلى الشمس في المجموعة الشمسية؟',
    options: ['عطارد', 'الزهرة', 'المريخ', 'الأرض'],
    correctIndex: 0,
    funFact: 'عطارد أصغر كواكب المجموعة الشمسية وأقربها للشمس، ويكمل دورته حولها في 88 يوماً أرضياً فقط.',
    rewardDinars: 10,
  },
  {
    id: 'sci_02',
    category: 'science',
    difficulty: 'easy',
    question: 'كم عدد كواكب المجموعة الشمسية؟',
    options: ['سبعة', 'تسعة', 'ثمانية', 'عشرة'],
    correctIndex: 2,
    funFact: 'أصبحت الكواكب ثمانية بعد أن أعاد الاتحاد الفلكي الدولي تصنيف بلوتو ككوكب قزم سنة 2006.',
    rewardDinars: 10,
  },
  {
    id: 'sci_03',
    category: 'science',
    difficulty: 'easy',
    question: 'ما هو أكبر كوكب في المجموعة الشمسية؟',
    options: ['زحل', 'أورانوس', 'نبتون', 'المشتري'],
    correctIndex: 3,
    funFact: 'المشتري عملاق غازي تتسع كتلته لأكثر من ألف أرض، ويشتهر ببقعته الحمراء العظيمة وهي عاصفة مستمرة منذ قرون.',
    rewardDinars: 10,
  },
  {
    id: 'sci_04',
    category: 'science',
    difficulty: 'easy',
    question: 'ما هو أقرب نجم إلى كوكب الأرض؟',
    options: ['نجم القطب', 'قلب العقرب', 'الشعرى اليمانية', 'الشمس'],
    correctIndex: 3,
    funFact: 'الشمس نجم متوسط الحجم يبعد عن الأرض نحو 150 مليون كيلومتر، ويصلنا ضوؤه في ثماني دقائق تقريباً.',
    rewardDinars: 10,
  },
  {
    id: 'sci_05',
    category: 'science',
    difficulty: 'medium',
    question: 'ما هو الغاز الذي يشكل النسبة الأكبر من الغلاف الجوي للأرض؟',
    options: ['النيتروجين', 'ثاني أكسيد الكربون', 'الأكسجين', 'الهيدروجين'],
    correctIndex: 0,
    funFact: 'يشكل النيتروجين نحو 78% من الغلاف الجوي، بينما لا يتجاوز الأكسجين 21%.',
    rewardDinars: 20,
  },
  {
    id: 'sci_06',
    category: 'science',
    difficulty: 'medium',
    question: 'ما هو المعدن الوحيد الذي يوجد سائلاً في درجة حرارة الغرفة؟',
    options: ['القصدير', 'الرصاص', 'الألمنيوم', 'الزئبق'],
    correctIndex: 3,
    funFact: 'الزئبق معدن سائل لامع كان يُستخدم في موازين الحرارة قديماً قبل أن يُقيَّد استخدامه لسميته.',
    rewardDinars: 20,
  },
  {
    id: 'sci_07',
    category: 'science',
    difficulty: 'easy',
    question: 'ما هو الغاز الذي تمتصه النباتات من الهواء لصنع غذائها؟',
    options: ['الأكسجين', 'النيتروجين', 'الميثان', 'ثاني أكسيد الكربون'],
    correctIndex: 3,
    funFact: 'غاز تطلقه المصانع والمركبات، وتمتصه النباتات في عملية البناء الضوئي.',
    rewardDinars: 10,
  },
  {
    id: 'sci_08',
    category: 'science',
    difficulty: 'easy',
    question: 'من أي عنصرين يتكون الماء؟',
    options: ['الكربون والأكسجين', 'الهيدروجين والأكسجين', 'النيتروجين والهيدروجين', 'الكالسيوم والأكسجين'],
    correctIndex: 1,
    funFact: 'صيغة الماء H₂O أي ذرتا هيدروجين وذرة أكسجين، وهو المذيب الأشهر في الطبيعة.',
    rewardDinars: 10,
  },
  {
    id: 'sci_09',
    category: 'science',
    difficulty: 'easy',
    question: 'عند أي درجة حرارة يغلي الماء عند مستوى سطح البحر؟',
    options: ['80 مئوية', '90 مئوية', '100 مئوية', '120 مئوية'],
    correctIndex: 2,
    funFact: 'يغلي الماء عند 100 درجة مئوية عند الضغط الجوي العادي، وتنخفض درجة الغليان كلما ارتفعنا عن سطح البحر.',
    rewardDinars: 10,
  },
  {
    id: 'sci_10',
    category: 'science',
    difficulty: 'medium',
    question: 'كم عدد العظام في جسم الإنسان البالغ؟',
    options: ['186 عظمة', '206 عظمة', '256 عظمة', '306 عظمة'],
    correctIndex: 1,
    funFact: 'يولد الإنسان بنحو 300 عظمة يلتحم بعضها مع النمو حتى تستقر عند 206 عظمات في سن البلوغ.',
    rewardDinars: 20,
  },
  {
    id: 'sci_11',
    category: 'science',
    difficulty: 'medium',
    question: 'ما هو أكبر عضو في جسم الإنسان؟',
    options: ['الكبد', 'الرئتان', 'الجلد', 'الأمعاء'],
    correctIndex: 2,
    funFact: 'الجلد أكبر أعضاء الجسم مساحةً ووزناً، ويقوم بدور الحاجز الواقي وتنظيم حرارة الجسم.',
    rewardDinars: 20,
  },
  {
    id: 'sci_12',
    category: 'science',
    difficulty: 'easy',
    question: 'ما هو العضو المسؤول عن ضخ الدم في جسم الإنسان؟',
    options: ['الكبد', 'القلب', 'الرئتان', 'الكلى'],
    correctIndex: 1,
    funFact: 'ينبض قلب الإنسان نحو 100 ألف نبضة في اليوم، ويضخ آلاف اللترات من الدم عبر الأوعية الدموية.',
    rewardDinars: 10,
  },
  {
    id: 'sci_13',
    category: 'science',
    difficulty: 'medium',
    question: 'من هو العالم الذي وضع قانون الجاذبية الأرضية؟',
    options: ['إسحاق نيوتن', 'ألبرت أينشتاين', 'غاليليو غاليلي', 'نيكولا تسلا'],
    correctIndex: 0,
    funFact: 'صاغ إسحاق نيوتن قانون الجذب العام في القرن السابع عشر، وهو أساس فهمنا لحركة الكواكب والأجسام.',
    rewardDinars: 20,
  },
  {
    id: 'sci_14',
    category: 'science',
    difficulty: 'hard',
    question: 'من هو العالم المسلم الذي يُعد رائد علم البصريات وصاحب "كتاب المناظر"؟',
    options: ['ابن الهيثم', 'ابن سينا', 'الخوارزمي', 'جابر بن حيان'],
    correctIndex: 0,
    funFact: 'وضع ابن الهيثم أسس المنهج التجريبي وفسّر آلية الإبصار تفسيراً علمياً سبق أوروبا بقرون.',
    hint: 'بصري المولد، وأول من فسّر الإبصار تفسيراً صحيحاً.',
    rewardDinars: 35,
    source: 'موسوعة المعرفة (مقال: ابن الهيثم)',
  },
  {
    id: 'sci_15',
    category: 'science',
    difficulty: 'hard',
    question: 'من هو الطبيب المسلم صاحب موسوعة "القانون في الطب"؟',
    options: ['ابن النفيس', 'الرازي', 'ابن سينا', 'الزهراوي'],
    correctIndex: 2,
    funFact: 'ظل "القانون في الطب" لابن سينا مرجعاً أساسياً في الجامعات الأوروبية لأكثر من ستة قرون.',
    hint: 'لُقب بالشيخ الرئيس.',
    rewardDinars: 35,
    source: 'موسوعة المعرفة (مقال: القانون في الطب)',
  },
  {
    id: 'sci_16',
    category: 'science',
    difficulty: 'hard',
    question: 'ما هي سرعة الضوء في الفراغ تقريباً؟',
    options: ['30 ألف كم/ث', '300 ألف كم/ث', '3 ملايين كم/ث', '30 مليون كم/ث'],
    correctIndex: 1,
    funFact: 'تبلغ سرعة الضوء نحو 299,792 كيلومتراً في الثانية، وهي السرعة القصوى في الكون وفق النسبية الخاصة.',
    hint: 'تقطع ما بين الأرض والقمر في ثانية وربع.',
    rewardDinars: 35,
    source: 'ويكيبيديا الموسوعة الحرة (مقال: سرعة الضوء)',
  },
  {
    id: 'sci_17',
    category: 'science',
    difficulty: 'medium',
    question: 'ما هو الكوكب الذي يُعرف بالكوكب الأحمر؟',
    options: ['المريخ', 'الزهرة', 'المشتري', 'عطارد'],
    correctIndex: 0,
    funFact: 'يكتسب المريخ لونه الأحمر من أكاسيد الحديد المنتشرة على سطحه، وهو أكثر الكواكب استقطاباً لبعثات الفضاء.',
    rewardDinars: 20,
  },
  {
    id: 'sci_18',
    category: 'science',
    difficulty: 'hard',
    question: 'من هو العالم المسلم الذي يُعد من مؤسسي علم الجبر، واشتُق من اسمه مصطلح رياضي وحاسوبي شهير؟',
    options: ['البيروني', 'الخوارزمي', 'الكندي', 'ثابت بن قرة'],
    correctIndex: 1,
    funFact: 'وضع محمد بن موسى الخوارزمي كتاب "الجبر والمقابلة"، ومنه انتقلت كلمة "الجبر" إلى اللغات الأوروبية، بينما جاءت كلمة "algorithm" من اسمه هو.',
    hint: 'عاش في بيت الحكمة ببغداد، ومن اسمه جاء مصطلح حاسوبي.',
    rewardDinars: 35,
    source: 'ويكيبيديا الموسوعة الحرة (مقال: محمد بن موسى الخوارزمي)',
  },
  {
    id: 'sci_19',
    category: 'science',
    difficulty: 'medium',
    question: 'لماذا نرى وميض البرق قبل أن نسمع صوت الرعد؟',
    options: [
      'لأن البرق يحدث قبل الرعد بدقائق',
      'لأن سرعة الضوء أكبر بكثير من سرعة الصوت',
      'لأن الصوت لا ينتقل في الهواء البارد',
      'لأن العين أسرع استجابة من الأذن',
    ],
    correctIndex: 1,
    funFact: 'يحدث البرق والرعد في اللحظة نفسها، لكن الصوت يحتاج نحو ثلاث ثوانٍ ليقطع كيلومتراً واحداً بينما يقطعه الضوء في جزء من مليون من الثانية.',
    rewardDinars: 20,
  },
  {
    id: 'sci_20',
    category: 'science',
    difficulty: 'medium',
    question: 'ما المرض الذي ينشأ عن انقسام غير منضبط لخلايا الجسم؟',
    options: ['السكري', 'الربو', 'السرطان', 'فقر الدم'],
    correctIndex: 2,
    funFact: 'تنقسم خلايا الجسم السليمة وفق نظام دقيق، وحين يختل هذا النظام تتكاثر الخلايا بلا ضابط فتتكوّن الأورام.',
    rewardDinars: 20,
  },
  {
    id: 'sci_21',
    category: 'science',
    difficulty: 'easy',
    question: 'ماذا نسمي المنطقة المحصورة داخل حدود شكل هندسي؟',
    options: ['المحيط', 'القطر', 'الوتر', 'المساحة'],
    correctIndex: 3,
    funFact: 'المساحة تقيس ما بداخل الشكل، بينما المحيط يقيس طول حدوده الخارجية — ولهذا يُقاس الأول بالوحدات المربعة.',
    rewardDinars: 10,
  },
  {
    id: 'sci_22',
    category: 'science',
    difficulty: 'easy',
    question: 'ماذا يُسمى المرض سريع الانتشار الذي يصيب أعداداً كبيرة في وقت قصير؟',
    options: ['الوباء', 'الحساسية', 'الالتهاب', 'الورم'],
    correctIndex: 0,
    funFact: 'يفرّق الأطباء بين الوباء الذي ينتشر في إقليم محدود، والجائحة التي تعبر القارات وتصيب العالم كله.',
    rewardDinars: 10,
  },
  {
    id: 'sci_23',
    category: 'science',
    difficulty: 'easy',
    question: 'كم عدد الحواس الأساسية عند الإنسان؟',
    options: ['خمس', 'ثلاث', 'أربع', 'ست'],
    correctIndex: 0,
    funFact: 'الحواس الخمس هي البصر والسمع والشم والذوق واللمس، ولكل منها عضو متخصص.',
    rewardDinars: 10,
  },
  {
    id: 'sci_24',
    category: 'science',
    difficulty: 'easy',
    question: 'ما أقرب جرم سماوي إلى كوكب الأرض؟',
    options: ['الشمس', 'القمر', 'الزهرة', 'المريخ'],
    correctIndex: 1,
    funFact: 'يبعد القمر عن الأرض نحو 384 ألف كيلومتر، ويدور حولها مرة كل شهر قمري تقريباً.',
    rewardDinars: 10,
  },
  {
    id: 'sci_25',
    category: 'science',
    difficulty: 'medium',
    question: 'ما الغاز الذي يحتاجه الإنسان من الهواء ليتنفس؟',
    options: ['النيتروجين', 'ثاني أكسيد الكربون', 'الأكسجين', 'الهيليوم'],
    correctIndex: 2,
    funFact: 'يشكل نحو 21% من هواء الغلاف الجوي، ولا تقوم حياة أغلب الكائنات بدونه.',
    rewardDinars: 20,
  },
  {
    id: 'sci_26',
    category: 'science',
    difficulty: 'medium',
    question: 'كم عدد الأسنان اللبنية عند الطفل؟',
    options: ['16 سناً', '24 سناً', '32 سناً', '20 سناً'],
    correctIndex: 3,
    funFact: 'تبدأ الأسنان اللبنية العشرون بالتساقط نحو السادسة، لتحل محلها الأسنان الدائمة الاثنتان والثلاثون.',
    rewardDinars: 20,
  },
  {
    id: 'sci_27',
    category: 'science',
    difficulty: 'medium',
    question: 'ما العضو الذي يفرز هرمون الأنسولين في جسم الإنسان؟',
    options: ['البنكرياس', 'الكبد', 'الطحال', 'الغدة الدرقية'],
    correctIndex: 0,
    funFact: 'ينظم الأنسولين مستوى السكر في الدم، وقصوره أو ضعف الاستجابة له هو أساس مرض السكري.',
    rewardDinars: 20,
  },
  {
    id: 'sci_28',
    category: 'science',
    difficulty: 'medium',
    question: 'ما وحدة قياس القوة في النظام الدولي للوحدات؟',
    options: ['الجول', 'النيوتن', 'الواط', 'الباسكال'],
    correctIndex: 1,
    funFact: 'سُميت وحدة القوة باسم إسحاق نيوتن، وتعادل القوة اللازمة لتسريع كيلوغرام واحد متراً في الثانية المربعة.',
    rewardDinars: 20,
  },
  {
    id: 'sci_29',
    category: 'science',
    difficulty: 'medium',
    question: 'ماذا تُسمى عملية تحول الماء من الحالة السائلة إلى بخار؟',
    options: ['التكاثف', 'التجمد', 'التبخر', 'الانصهار'],
    correctIndex: 2,
    funFact: 'التبخر أساس دورة الماء في الطبيعة: يتبخر من البحار ثم يتكاثف سحاباً ثم يعود مطراً.',
    rewardDinars: 20,
  },
  {
    id: 'sci_30',
    category: 'science',
    difficulty: 'medium',
    question: 'ما الكوكب المعروف بحلقاته الواضحة المحيطة به؟',
    options: ['المريخ', 'عطارد', 'الزهرة', 'زحل'],
    correctIndex: 3,
    funFact: 'تتكون حلقات زحل من مليارات القطع الجليدية والصخرية، وهي أوضح حلقات كوكبية تُرى بالمقراب.',
    rewardDinars: 20,
  },
  {
    id: 'sci_31',
    category: 'science',
    difficulty: 'hard',
    question: 'ما العنصر الأكثر وفرة في القشرة الأرضية؟',
    options: ['الأكسجين', 'الحديد', 'السيليكون', 'الألمنيوم'],
    correctIndex: 0,
    funFact: 'يشكل الأكسجين نحو نصف كتلة القشرة الأرضية، لأنه داخل في تركيب معظم الصخور والمعادن.',
    hint: 'نحو نصف وزن القشرة، وهو غاز في حالته الحرة.',
    source: 'ويكيبيديا العربية (مقال: قشرة (جيولوجيا))',
    rewardDinars: 35,
  },
  {
    id: 'sci_32',
    category: 'science',
    difficulty: 'hard',
    question: 'كم عدد الكروموسومات في خلية الإنسان الطبيعية؟',
    options: ['23', '46', '48', '64'],
    correctIndex: 1,
    funFact: 'الكروموسومات 46 في 23 زوجاً، يأتي نصفها من الأب ونصفها من الأم، وآخر زوج يحدد الجنس.',
    hint: 'ثلاثة وعشرون زوجاً.',
    source: 'ويكيبيديا العربية (مقال: صبغي)',
    rewardDinars: 35,
  },
  {
    id: 'sci_33',
    category: 'science',
    difficulty: 'hard',
    question: 'ما اسم أكبر شريان في جسم الإنسان؟',
    options: ['الشريان التاجي', 'الشريان السباتي', 'الأبهر (الأورطي)', 'الشريان الرئوي'],
    correctIndex: 2,
    funFact: 'يخرج الأبهر من القلب مباشرة ليوزع الدم المؤكسج على الجسم كله عبر فروعه.',
    hint: 'يخرج مباشرة من البطين الأيسر.',
    source: 'ويكيبيديا العربية (مقال: أبهر (شريان))',
    rewardDinars: 35,
  },
  {
    id: 'sci_34',
    category: 'science',
    difficulty: 'hard',
    question: 'من العالم المسلم الذي وصف الدورة الدموية الصغرى قبل الأوروبيين بقرون؟',
    options: ['الرازي', 'الزهراوي', 'البيروني', 'ابن النفيس'],
    correctIndex: 3,
    funFact: 'وصف ابن النفيس في القرن الثالث عشر مرور الدم من القلب إلى الرئتين، قبل وليم هارفي بنحو ثلاثة قرون.',
    hint: 'دمشقي عمل في مصر، وشرح تشريح القانون.',
    source: 'ويكيبيديا العربية (مقال: ابن النفيس)',
    rewardDinars: 35,
  },
  {
    id: 'sci_35',
    category: 'science',
    difficulty: 'hard',
    question: 'بأي مقياس تُقدَّر قوة الزلازل؟',
    options: ['مقياس ريختر', 'مقياس سلسيوس', 'مقياس بوفورت', 'مقياس ديسيبل'],
    correctIndex: 0,
    funFact: 'مقياس ريختر لوغاريتمي، فكل درجة تعني طاقة أكبر بنحو اثنتين وثلاثين مرة من الدرجة التي قبلها.',
    hint: 'يحمل اسم عالم أمريكي وضعه سنة 1935.',
    source: 'ويكيبيديا العربية (مقال: مقياس ريختر)',
    rewardDinars: 35,
  },
  {
    id: 'sci_36',
    category: 'science',
    difficulty: 'hard',
    question: 'ما الغاز النبيل الذي يُملأ به أنبوب الإضاءة فيعطي وهجاً أحمر برتقالياً؟',
    options: ['الهيليوم', 'النيون', 'الأرجون', 'الكريبتون'],
    correctIndex: 1,
    funFact: 'لافتات "النيون" سميت باسم هذا الغاز، ولكل غاز نبيل لون توهج مختلف عند تفريغ الكهرباء فيه.',
    hint: 'اسمه يوناني معناه «الجديد».',
    source: 'ويكيبيديا العربية (مقال: نيون)',
    rewardDinars: 35,
  },
  {
    id: 'sci_37',
    category: 'science',
    difficulty: 'expert',
    question: 'ما الرمز الكيميائي لعنصر الذهب في الجدول الدوري؟',
    options: ['Ag', 'Gd', 'Au', 'Go'],
    correctIndex: 2,
    funFact: 'الرمز Au مأخوذ من اسمه اللاتيني Aurum، بينما Ag هي الفضة من Argentum.',
    hint: 'من الاسم اللاتيني aurum.',
    rewardDinars: 60,
    source: 'الجدول الدوري للعناصر — الرمز Au من اللاتينية Aurum',
  },
  {
    id: 'sci_38',
    category: 'science',
    difficulty: 'expert',
    question: 'كم عدد العناصر المعتمدة في الجدول الدوري حالياً؟',
    options: ['92 عنصراً', '104 عناصر', '110 عناصر', '118 عنصراً'],
    correctIndex: 3,
    funFact: 'آخر أربعة عناصر اعتُمدت رسمياً سنة 2016 لتكتمل الدورة السابعة من الجدول الدوري.',
    hint: 'آخر أربعة منها أُقرّت أسماؤها سنة 2016.',
    rewardDinars: 60,
    source: 'الاتحاد الدولي للكيمياء البحتة والتطبيقية (IUPAC) — الجدول الدوري',
  },
  {
    id: 'sci_39',
    category: 'science',
    difficulty: 'expert',
    question: 'من العالم الذي أرسى أسس علم الوراثة بتجاربه على نبات البازلاء؟',
    options: ['غريغور مندل', 'تشارلز داروين', 'لويس باستور', 'روبرت هوك'],
    correctIndex: 0,
    funFact: 'نشر مندل نتائجه سنة 1866 فمرت دون اهتمام، ولم يُعرف قدرها إلا بعد وفاته بعقود.',
    hint: 'راهب نمساوي عمل في حديقة ديره.',
    source: 'ويكيبيديا العربية (مقال: غريغور مندل)',
    rewardDinars: 60,
  },
  {
    id: 'sci_40',
    category: 'science',
    difficulty: 'expert',
    question: 'ما العملية التي تُنتج بها الشمس طاقتها الهائلة؟',
    options: ['الاحتراق الكيميائي', 'الاندماج النووي', 'الانشطار النووي', 'التحلل الإشعاعي'],
    correctIndex: 1,
    funFact: 'تندمج نوى الهيدروجين في قلب الشمس لتكوّن الهيليوم، فتتحول فروق الكتلة إلى طاقة هائلة.',
    hint: 'تتحد فيها نوى الهيدروجين لتصير هيليوم.',
    source: 'ويكيبيديا العربية (مقال: الشمس)',
    rewardDinars: 60,
  },
  {
    id: 'sci_nal_02',
    category: 'science',
    cityId: 'nalut_nafusa',
    difficulty: 'medium',
    question: 'ما هو الاكتشاف العلمي الجيولوجي النادر الذي اشتهرت به جبال نالوت في جبل نفوسة وخُصص له متحف ومجسم تذكاري عند مدخل المدينة؟',
    options: ['أحافير وعظام ديناصورات', 'نيازك قمرية', 'ينابيع مياه كبريتية حارة', 'كهوف جليدية'],
    correctIndex: 0,
    funFact: 'أظهرت الاكتشافات الجيولوجية بقايا عظام متحجرة لديناصورات عاشت في تلك المرتفعات خلال العصر الطباشيري قبل عشرات ملايين السنين.',
    hint: 'كائنات عملاقة منقرضة عاشت في عصور ما قبل التاريخ عُثر على بقايا هياكلها في صخور الجبل.',
    rewardDinars: 20,
    source: 'موسوعة ويكيبيديا العربية (مادة: نالوت)',
  },
  {
    id: 'sci_mis_02',
    category: 'science',
    cityId: 'misrata',
    difficulty: 'easy',
    question: 'في أي مدينة ليبية يقع المجمع الصناعي الأكبر في البلاد لإنتاج وتصنيع الحديد والصلب (LISCO)؟',
    options: ['مصراتة', 'طبرق', 'الزاوية', 'درنة'],
    correctIndex: 0,
    funFact: 'يمتد مجمع الحديد والصلب في منطقة قصر أحمد على مساحة تزيد عن 1200 هكتار ويمتلك ميناءه التجاري الخاص.',
    hint: 'مدينة ساحلية تجارية كبرى في غرب ليبيا تشتهر بمنطقة قصر أحمد.',
    rewardDinars: 10,
    source: 'موسوعة ويكيبيديا العربية (مادة: الشركة الليبية للحديد والصلب)',
  },
  {
    id: 'sci_cyr_02',
    category: 'science',
    cityId: 'cyrene_green_mountain',
    difficulty: 'medium',
    question: 'من هو العالم الموسوعي الشهير الذي وُلد في مدينة قورينا (شحات) في القرن الثالث قبل الميلاد ويُعد أول من قام بحساب محيط كوكب الأرض بدقة مذهلة؟',
    options: ['إراتوستينس', 'أرخميدس', 'بطليموس', 'فيثاغورس'],
    correctIndex: 0,
    funFact: 'حسب إراتوستينس محيط الأرض عبر قياس زاوية سقوط الأشعة والظلال بين أسوان والإسكندرية في يوم الانقلاب الصيفي.',
    hint: 'عالم جغرافي وفلكي قوريني تولى أمانة مكتبة الإسكندرية القديمة وله غربال شهير للأعداد الأولية.',
    rewardDinars: 20,
    source: 'موسوعة ويكيبيديا العربية (مادة: إراتوستينس)',
  },
  {
    id: 'sci_ght_02',
    category: 'science',
    cityId: 'ghat_akakus',
    difficulty: 'expert',
    question: 'ما اسم المأوى الصخري الأثري في جبال تدرارت أكاكوس قرب غات، الذي كشفت حفرياته عن أقدم تسلسل لاستئناس الحيوانات والمومياوات في الصحراء الكبرى؟',
    options: ['تاكاركوري', 'وان محجّاج', 'قرزة', 'جرمة'],
    correctIndex: 0,
    funFact: 'مكّنت الأنسجة المحفوظة لرفات تم العثور عليها في تاكاركوري العلماء من استخلاص وتحليل أول حمض نووي قديم من عمق الصحراء الإفريقية.',
    hint: 'موقع في أكاكوس أعطى علماء الجينات أقدم ما استُخرج من الصحراء.',
    rewardDinars: 60,
    source: 'موسوعة ويكيبيديا العربية (مادة: تاكاركوري)',
  },
];
